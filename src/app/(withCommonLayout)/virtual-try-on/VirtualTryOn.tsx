...
// FIX 1: Removed /public/ prefix — Next.js serves from root, not /public/
...
export default function VirtualTryOn() {
  ...
  const [captureMode, setCaptureMode] = useState<"live" | "demo" | "upload">("demo");
  ...
  // Overlay glasses onto a static image URL using canvas
  const overlayGlassesOnImage = useCallback(async (
    imageSrc: string,
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        glassesImg.onload = () => {
          // Heuristic: place glasses in the upper-center 40% of the image
          const glassesWidth = img.width * 0.5;
          const aspectRatio = glassesImg.height / glassesImg.width;
          const glassesHeight = glassesWidth * aspectRatio;
          const glassesX = (img.width - glassesWidth) / 2;
          const glassesY = img.height * 0.22; // roughly eye level
  ...
  // FIX 4: overlayGlasses using actual landmark data (for live mode)
  const overlayGlassesWithLandmarks = useCallback(async (
    imageSrc: string,
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        glassesImg.onload = () => {
          ...
          const glassesHeight = glassesImg.height * (glassesWidth / glassesImg.width);
          ...
          const glassesY = centerY - glassesHeight * 0.4;
  ...
  useEffect(() => {
    if (captureMode !== "live" || !webcamRef.current?.video || !selectedProduct) return;
    ...
    const initializeFaceMesh = async () => {
      try {
            ...
            if (outputCanvas) {
              ...
              outputImg.onload = () => {
                if (outputCtx) {
                  ...
                  outputCtx.drawImage(outputImg, 0, 0, outputCanvas.width, outputCanvas.height);
                }
              };
            }
          } catch (err) {
        ...
        setError("Failed to initialize camera. Please try again or use demo mode.");
      }
    };
  ...
  const applyGlassesToDemo = useCallback(async (imageUrl: string, product: Product | null) => {
    if (!product) {
    ...
    try {
      const result = await overlayGlassesOnImage(imageUrl, product.glassesImage);
      setProcessedImage(result);
    } catch (err) {
      console.error("Error applying glasses to demo:", err);
      setError("Failed to overlay glasses. Check that the glasses image exists in /public/.");
  ...
  const handleDemoSelect = useCallback(async (imageUrl: string) => {
  ...
  const handleProductSelect = useCallback((product: Product) => {
    ...
    setRecentlyTried(prev => [product.id, ...prev.filter(id => id !== product.id)].slice(0, 3));
    setProcessedImage(null);
    ...
    <section className="relative w-full overflow-hidden bg-gradient-to-b from-black via-gray-900 to-black py-12 sm:py-16 lg:py-20 px-4 sm:px-6">
      ...
      <div className="absolute inset-0 opacity-5">
        <div
          ...
          style={{
            backgroundImage: "radial-gradient(circle at 2px 2px, #007C74 1px, transparent 0)",
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xs sm:text-sm md:text-base text-neutral-300 max-w-2xl mx-auto">
            See how our sunglasses look on real models or yourself with AI-powered try-on
          </p>
        </motion.div>
...
/>
                ...
                <>
                  ...
                  {!selectedProduct && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-3">
                      <p className="text-sm text-neutral-300">Select a product to try on</p>
                        ...
                        {demoModels.map((model) => (
                          <motion.button
                            ...
                            className={`relative w-14 h-14 rounded-lg overflow-hidden ring-2 transition-all ${
                              selectedDemo === model.url ? "ring-[#007C74]" : "ring-transparent hover:ring-white/40"
                            }`}
                          >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                {isProcessing && (
                  ...
                  >
                    <div className="text-center">
                      ...
                      <p className="text-xs text-neutral-400">Overlaying glasses on face</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3 flex items-center justify-between pointer-events-none">
                <div className="flex gap-1 sm:gap-2 pointer-events-auto">
                  <motion.button
                    ...
                    title={captureMode === "live" ? "Switch to Demo" : "Use Live Camera"}
                  >
          ...
          <div className="lg:col-span-1">
            <div className="h-full rounded-xl sm:rounded-2xl backdrop-blur-sm bg-white/5 border border-white/10 p-3 sm:p-4">
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <h3 className="text-xs sm:text-sm font-semibold text-white">Select Your Style</h3>
                <span className="text-[10px] sm:text-xs text-neutral-400">{products.length} styles</span>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mt-8 sm:mt-10 lg:mt-12">
          {[
            ...
            >
              ...
              <p className="text-[10px] sm:text-xs text-neutral-400">{feature.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-white/10">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] sm:text-xs text-neutral-400">98% customer satisfaction</span>
          </div>
        </div>
